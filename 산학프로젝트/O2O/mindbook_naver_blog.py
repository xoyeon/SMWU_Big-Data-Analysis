...
if __name__ == "__main__":
    ...
    # food_keyword = "곱창"
    # ontology1 = "쫄깃쫄깃"
    # search_word = f'"{food_keyword}" "{ontology1}"'
    ...
    """
    df_ontology = pd.read_excel("mindbook_menu_list_test.xlsx", engine = "openpyxl", sheet_name="ontology")
    df_food_list = pd.read_excel("mindbook_menu_list_test.xlsx", engine = "openpyxl", sheet_name="food_list")
    # print(df_ontology)
    # print(df_food_list)
    ...
    * 음식속성 컬럼명(list)
    ...
    for each_food in tqdm(list_food_list, desc="음식 속성 네이버 API 요청중..."): # 개별 음식 리스트 추출
        ...
        for each_ontology in list_ontology_column: # 개별 속성 추출
            ...
            for each_keyword in list_each_ontology_keywords:
                ...
                search_word = f'"{each_food}" "{each_keyword}"' # 네이버 블로그 검색 문장
    ...
    df_food_ontology_raw_value.to_csv('df_food_ontology.csv', encoding='utf-8-sig', index=False)
    df_food_ontology_raw_value.to_excel(excel_writer='df_food_ontology.xlsx')
    ...
    df_food_ontology_percenet_value.to_csv('df_food_ontology_percent.csv', encoding='utf-8-sig', index=False)
    df_food_ontology_raw_value.to_excel(excel_writer='df_food_ontology_percent.xlsx')